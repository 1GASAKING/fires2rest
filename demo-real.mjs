...
// ============================================================
const db = Firestore.useServiceAccount(
    projectId,
    {
        clientEmail,
...
// Path we'll test (change this to a real path that exists in your project!)
const candidates = [
    "users/test-user",
    "users/alice",
    "test/test-doc",
    "fires2rest-demo/demo-doc",
];
...
for (const path of candidates) {
    try {
        ...
        if (snap.exists) {
            ...
            console.log(`   (not found, trying next path...)`);
        }
    } catch (err) {
...
try {
    ...
    console.log(`\n🗑️  Cleaning up: deleting ${demoDocPath}`);
    await db.doc(demoDocPath).delete();
    console.log("   ✅ Deleted.");
} catch (err) {
